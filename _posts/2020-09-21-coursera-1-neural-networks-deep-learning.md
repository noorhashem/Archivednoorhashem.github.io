...
the intution behind this is that for a given w value, if at a given value it yields a high result for J, then the derivative of J with respect to this w value is gonna be positive (positive slop) so subtracting it from w will decrease the value of w and b used untill we reach a value at which J(w,b) is at its minimum. 
